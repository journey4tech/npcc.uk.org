...
class Auth_controller extends CI_Controller
{
    ...
    public function login()
    {
        ...
        $data['description'] = trans("login") . " - " . $this->config->item('application_name');
        $data['keywords']    = trans("login") . "," . $this->config->item('application_name');
    ...
    public function register()
    {
        ...
        $data['description'] = trans("register") . " - " . $this->config->item('application_name');
        $data['keywords']    = trans("register") . "," . $this->config->item('application_name');
    ...
    public function register_post()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if (!$this->auth_model->is_unique_email($email)) {
                $this->session->set_flashdata('form_data', $this->auth_model->input_values());
                $this->session->set_flashdata('error',"The email has already been taken.");
                redirect($this->agent->referrer());
            }
            //is username unique
            ...
            if ($user_id) {
                ...
                if (!empty($user)) {
                    ...
                    if ($this->config->item('email_verification') != 1) {
    ...
    public function confirm_email()
    {
        ...
        $data['description'] = trans("confirm_your_account") . " - " . $this->config->item('application_name');
        $data['keywords']    = trans("confirm_your_account") . "," . $this->config->item('application_name');
        ...
        if ($this->auth_model->verify_email($data["user"])) {
            $data["success"] = "Your email address has been successfully confirmed!";
        } else {
    ...
    public function forgot_password()
    {
        ...
        $data['description'] = trans("reset_password") . " - " . $this->config->item('application_name');
        $data['keywords']    = trans("reset_password") . "," . $this->config->item('application_name');
    ...
    public function forgot_password_post()
    {
        ...
        //if user not exists
        if (empty($user)) {
            $this->session->set_flashdata('error', "We can't find a user with that e-mail address!");
            redirect($this->agent->referrer());
        } else {
