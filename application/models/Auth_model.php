...
class Auth_model extends CI_Model
{
	...
	public function login()
	{
		...
		if (!empty($user)) {
			...
			if ($user->email_status != 1) {
				$this->session->set_flashdata('error', "In order to login to the site, you must confirm your email address." . "&nbsp;<a href='javascript:void(0)' class='link-resend-activation-email' onclick=\"send_activation_email('" . $user->id . "','" . $user->token . "');\">" . trans("resend_activation_email") . "</a>");
				return false;
			}
			if ($user->banned == 1) {
	...
	public function login_direct($user)
	{
		...
		$user_data = array(
			...
			'user_role' => $user->role,
			'is_user_login' => true,
			'app_key' => $this->config->item('application_name'),
		);
	...
	public function register()
	{
		...
		if ($this->db->insert('users', $data)) {
			...
			if ($this->config->item('email_verification') == 1) {
				...
				if (!empty($user)) {
					$this->session->set_flashdata('success',"Your account has been created successfully!" . " " . "A confirmation email has been sent to your email address for activation. Please confirm your account." . "&nbsp;<a href='javascript:void(0)' class='link-resend-activation-email' onclick=\"send_activation_email_register('" . $user->id . "','" . $user->token . "');\">" . "Resend Activation Email" . "</a>");
					$this->send_email_activation($user->id, $user->token);
				}
			}
			return $last_id;
		} else {
		...
		if (!empty($user_id)) {
			...
			if (!empty($user)) {
				...
				$email_data = array(
					...
					'email_content' => "Please click on the button below to confirm your email.",
					'email_link' => base_url() . "confirm?token=" . $data['token'],
